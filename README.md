...
```bash
cargo test                                         # 333 tests, no network required
...
was read, a magnitude, and a **written falsifier**. Twenty-three entries. Each magnitude below is quoted from the filing rather than summed, because
...
| AMZN → Lambda | **0** | read and **REFUTED** — "Lambda" is **AWS's own product** |
